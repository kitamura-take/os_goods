...
display_edit_contensts(){
  ...
  if [ ${#check_sec_last_char} -ge 1 ]; then
    local edit_label="$(echo "${edit_label}" | sed -e "$ a \ \n")"
  fi
  ...
  local edit_label=$(echo "${source_cmd_display}" | sed -e "s/${CMDCLICK_N_CAHR}/n/g" -e 's/'${CMDCLICK_BACKSLASH_MARK}'/\\/g' -e 's/'${CMDCLICK_ONE_BACKSLASH_MARK}'/\\/g')
  local edit_label=$(cat <(echo " ") <(echo "please edit bellow command;") <(echo " ") <(echo "${edit_label}"))
  # lecho  "edit_label: ${edit_label}"
  local edit_label="$(echo "${edit_label}" | sed '1d' | sed -e 's/^#.*//' -e "s/^[a-zA-Z0-9_-]\{1,100\}=.*//" | sed '/^$/d')"
  #ウィンドウサイズ策定 
